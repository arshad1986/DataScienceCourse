import mincemeat
import operator
import os
import sys
import ast
import pickle
import re

def big_data_map(k, v):
        print "map function"
        for values in v:
                yield (k,values),1

        

def big_data_reduce(k, v):
        print "reduce function"
        return len(v)


allStopWords={'about':1, 'above':1, 'after':1, 'again':1, 'against':1, 'all':1, 'am':1, 'an':1, 'and':1, 'any':1, 'are':1, 'arent':1, 'as':1, 'at':1, 'be':1, 'because':1, 'been':1, 'before':1, 'being':1, 'below':1, 'between':1, 'both':1, 'but':1, 'by':1, 'cant':1, 'cannot':1, 'could':1, 'couldnt':1, 'did':1, 'didnt':1, 'do':1, 'does':1, 'doesnt':1, 'doing':1, 'dont':1, 'down':1, 'during':1, 'each':1, 'few':1, 'for':1, 'from':1, 'further':1, 'had':1, 'hadnt':1, 'has':1, 'hasnt':1, 'have':1, 'havent':1, 'having':1, 'he':1, 'hed':1, 'hell':1, 'hes':1, 'her':1, 'here':1, 'heres':1, 'hers':1, 'herself':1, 'him':1, 'himself':1, 'his':1, 'how':1, 'hows':1, 'i':1, 'id':1, 'ill':1, 'im':1, 'ive':1, 'if':1, 'in':1, 'into':1, 'is':1, 'isnt':1, 'it':1, 'its':1, 'its':1, 'itself':1, 'lets':1, 'me':1, 'more':1, 'most':1, 'mustnt':1, 'my':1, 'myself':1, 'no':1, 'nor':1, 'not':1, 'of':1, 'off':1, 'on':1, 'once':1, 'only':1, 'or':1, 'other':1, 'ought':1, 'our':1, 'ours ':1, 'ourselves':1, 'out':1, 'over':1, 'own':1, 'same':1, 'shant':1, 'she':1, 'shed':1, 'shell':1, 'shes':1, 'should':1, 'shouldnt':1, 'so':1, 'some':1, 'such':1, 'than':1, 'that':1, 'thats':1, 'the':1, 'their':1, 'theirs':1, 'them':1, 'themselves':1, 'then':1, 'there':1, 'theres':1, 'these':1, 'they':1, 'theyd':1, 'theyll':1, 'theyre':1, 'theyve':1, 'this':1, 'those':1, 'through':1, 'to':1, 'too':1, 'under':1, 'until':1, 'up':1, 'very':1, 'was':1, 'wasnt':1, 'we':1, 'wed':1, 'well':1, 'were':1, 'weve':1, 'were':1, 'werent':1, 'what':1, 'whats':1, 'when':1, 'whens':1, 'where':1, 'wheres':1, 'which':1, 'while':1, 'who':1, 'whos':1, 'whom':1, 'why':1, 'whys':1, 'with':1, 'wont':1, 'would':1, 'wouldnt':1, 'you':1, 'youd':1, 'youll':1, 'youre':1, 'youve':1, 'your':1, 'yours':1, 'yourself':1, 'yourselves':1}

# read files 
s_file_dir = sys.argv[1]



'''with open(stopword_file) as f:
        data = f.readlines()
        stopwords_dict = ast.literal_eval(data)
        print stopwords_dict
        print type(stopwords_dict)
        #stopwords_dict = ast.literal_eval(data)
'''

print "Source directory: " + s_file_dir
file_dir = os.listdir(s_file_dir)
files_remaining = len(file_dir)
dict_data = {}

for files in file_dir:
        print "Processing File:" + "----------->" + files
        files_remaining-=1
        files_path = os.path.join(s_file_dir,files)
        #print "file:" + 
        with open(files_path) as f:
                s_unfiltered_data = f.readlines()
                print "Processing line: " + "----------->>>>>>>   " 
                print  s_unfiltered_data
                for line in s_unfiltered_data:
                        s_splitdata = line.split(':::')
                        s_authors = s_splitdata[1].split('::')
                        #splitting with whitespace as delimeter and removing whitespace in words  using sptrip 
                        #s_title_unclean = s_splitdata[2].strip().split()
                        Data = s_splitdata[2].strip()
                        s_title_unclean = re.findall(r"[\w']+", Data)
                        # removing any full-stop or colon from words
                        s_title = [(word.strip(":.,")).lower() for word in s_title_unclean]
                        s_title_stopword_removed =[]
                        s_title_stopword_removed[:] = [word for word in s_title if((allStopWords.get(word,None))==None) and (len(word)>1)]
                        #print "Type or s_title"
                        print s_title
                        #print "author(s):",s_authors,"title:",s_title
                        for author in s_authors:
                                #print "multiple_authors:currently processing" + author
                                if (len(dict_data.keys())==0):
                                        #print "empty data_dict"
                                        dict_data[author] = s_title_stopword_removed
                                        #print dict_data 
                                        #print "********processing done*****"
                                elif author in dict_data.keys():
                                        #print author + "author already exists" 
                                        s_existing_title = dict_data[author]
                                        #print "existing title " 
                                        #print  s_existing_title
                                        s_existing_title.extend(s_title_stopword_removed)
                                        #print (s_title),(s_extended_title_list)
                                        dict_data[author] = s_existing_title
                                        #print dict_data[author]
                                        #print "******multiple author entry ********"
                                else:
                                        print "author does not exists in dict and dict is not empty"+"******"
                                        dict_data[author] = s_title_stopword_removed
                                        #print dict_data
                        print "********Line processed*****"
        print "File processed successfully:" + "----->>" + files
        print "files remaining ::::::::::::::::::" + str(files_remaining) 
print dict_data
                
                #print s_unfiltered_data

s = mincemeat.Server()
s.datasource = dict_data

s.mapfn = big_data_map
s.reducefn = big_data_reduce
results = s.run_server(password="bigdata")

print "FINAL RESULTS::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::"
print results



sorted_results = sorted(results.iteritems(), key=operator.itemgetter(1))
sorted_results.reverse()
for freq in sorted_results[:]:
        print freq
